...
{
    return _GetGuildBankTabPrice(tabId) * GOLD;;
}
...
void Guild::SetLevel(uint8 level, bool byCommand)
{
    ...
    char worldMsg[250];
    sprintf(worldMsg, "La Hermandad %s ha conseguido el nivel %d.", m_name.c_str(), level);
...
bool Guild::CanTakePromo()
{
    if (m_timePromocionGuildActive == NULL)
        return false;
    //Si el tiempo final de la promo es menor que ahora, la promocion esta activa
    return m_timePromocionGuildActive > time(0);
}

void Guild::_LoadPromoFromDB()
{
    QueryResult result = CharacterDatabase.PQuery("select promoGuild from guild  where guildid = %u", GetId());
    if (result)
    {
        Field* field = result->Fetch();
        m_timePromocionGuildActive = field[0].GetUInt32();
    }
    else
        m_timePromocionGuildActive = NULL;
}
void Guild::ActivatePromo(uint32 nDays)
{
    m_timePromocionGuildActive = ((24 * 60 * 60) * nDays) + time(0);
    QueryResult result = CharacterDatabase.PQuery("update guild set promoGuild = %u where guildid = %u", m_timePromocionGuildActive, GetId());
}

uint32 Guild::GetOnlineCount(uint32 nMinLevel)
{
    uint32 ret = 0;
    for (auto itr = m_members.begin(); itr != m_members.end(); ++itr)
        if (Player* player = itr->second->FindConnectedPlayer())
        {
            if (player->getLevel() >= nMinLevel && !player->HasPromoInAccount())
                ret++;
        }

    return ret;
}
void Guild::ApplyDisbandDebuff()
{
    for (auto itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (Player* player = itr->second->FindConnectedPlayer())
        {
            if (player->HasPromoGuild())
                player->ActivatePromo(7);
        }
    }

    QueryResult result = CharacterDatabase.PQuery("update promocion_guild set penalizacion = %u where idGuild = %u", time(0), GetId());
}
