...
bool SpellArea::IsFitToRequirements(Player const* player, uint32 newZone, uint32 newArea) const
{
    ...
    if (classMask)                                // is not expected class
        if (!player || !(classMask & player->getClassMask()))
            return false;
...
void SpellMgr::LoadSpellProcs()
{
    ...
    if (result)
    {
        do
        {
            ...
            baseProcEntry.idSpell = fields[0].GetInt32();
...
void SpellMgr::LoadSpellAreas()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT spell, area, quest_start, quest_start_status, quest_end_status, quest_end, aura_spell, racemask,classmask, gender, autocast FROM spell_area");
    if (!result)
    {
    ...
    do
    {
        ...
        spellArea.classMask = fields[8].GetUInt32();
...

        {
            ...
            for (SpellAreaMap::const_iterator itr = sa_bounds.first; itr != sa_bounds.second; ++itr)
            {
                ...
                if ((spellArea.raceMask & itr->second.raceMask) == 0)
                    continue;
                if ((spellArea.classMask & itr->second.classMask) == 0)
                    continue;
        ...
        if (spellArea.classMask && (spellArea.classMask & CLASSMASK_ALL_PLAYABLE) == 0)
        {
            TC_LOG_ERROR("sql.sql", "The spell %u listed in `spell_area` has wrong race mask (%u) requirement.", spell, spellArea.raceMask);
            continue;
        }
...
void SpellMgr::LoadSpellInfoStore()
{
    ...
    for (uint32 spellIndex = 0; spellIndex < GetSpellInfoStoreSize(); ++spellIndex)
    {
        ...
        //Turrado Pendiente de revisar y aceptar el assert
...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    // Warrior Grand Champion - Rolling Throw
    // Should hit both caster and target
    ApplySpellFix({ 67546 }, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ENEMY);
    });
    // The Black Knight - Death's Push (casted on announcer)
    ...
    //TURRADO Comento este Fix para habilitar las monturas con nuestra propia autoequitacion
    if (false)
    {
        ApplySpellFix({
            47977, // Magic Broom
