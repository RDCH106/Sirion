...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    if (Player* player = mover->ToPlayer())
    {
        //Marco la montura
        uint32 effect = 32;
        if (spellInfo->Effects[0].ApplyAuraName == effect || spellInfo->Effects[1].ApplyAuraName == effect || spellInfo->Effects[2].ApplyAuraName == effect)
        {
            _player->SetUInt32Value(PLAYER_MOUNT_ID, spellId);
