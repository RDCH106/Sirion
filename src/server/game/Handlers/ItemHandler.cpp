        ...
        if (pProto)
        {
            if (pProto->SellPrice > 0)
            {
                if (count < pItem->GetCount())               // need split items
                {
                    ...
                    TC_LOG_INFO("entities.player.items", "Player::SoldItem: Player '%s' (%s), Item: %u, Count: %u",
                        _player->GetName().c_str(), _player->GetGUID().ToString().c_str(), pItem->GetEntry(), count);
...
void WorldSession::HandleBuybackItem(WorldPacket& recvData)
{
    ...
    if (pItem)
    {
        ...
        if (msg == EQUIP_ERR_OK)
        {
            //Revisar porque hace crash _player->GetGUID().ToString().c_str()
            //Lo muevo arriba por si acaso el puntero al item desaparece o se modifica tras el StoreItem
            TC_LOG_INFO("entities.player.items", "Player::ReCompraNPC: Player '%s' (%s), Item: %u , Count: %u",
                _player->GetName().c_str(), _player->GetGUID().ToString().c_str(), pItem->GetEntry(), pItem->GetCount());
