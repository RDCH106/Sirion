...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
    ...
    if (pBattleGround && action)
        SendChatMessage("%sEstas jugando por la %s%s en %s", MSG_COLOR_WHITE, GetTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"Alianza" : MSG_COLOR_RED"Horda", MSG_COLOR_WHITE, pBattleGround->GetName().c_str());
}
