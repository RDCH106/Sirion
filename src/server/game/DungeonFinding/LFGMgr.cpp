    ...
    LfgLockMap const LFGMgr::GetLockedDungeons(ObjectGuid guid)
    {
        ...
        for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
        {
            ...
            else if (AccessRequirement const* ar = sObjectMgr->GetAccessRequirement(dungeon->map, Difficulty(dungeon->difficulty)))
            {
                ...
                //TURRADO
                //Añado el segundo logro de requisito
                else if (ar->achievement1 && ar->achievement2 && !player->HasAchieved(ar->achievement1) && !player->HasAchieved(ar->achievement2)
                    || ar->achievement1 && !ar->achievement2 && !player->HasAchieved(ar->achievement1) || !ar->achievement1 && ar->achievement2 && !player->HasAchieved(ar->achievement2))
