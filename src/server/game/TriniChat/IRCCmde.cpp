...
void IRCCmd::Account_Player(_CDATA *CD)
{
	...
	ObjectGuid guid = ObjectAccessor::FindPlayerByName(_PARAMS[0])->GetGUID();
	int account_id = 0;
	account_id = ObjectAccessor::FindPlayerByLowGUID(guid)->GetSession()->GetAccountId();
	if (account_id)
	{
...
{
	char playersNum[50];
	sprintf(playersNum, "%u", sWorld->GetPlayerCount());
	char maxPlayersNum[50];
	sprintf(maxPlayersNum, "%u", sWorld->GetMaxPlayerCount());
	...
	std::string updateTime = std::to_string(sWorldUpdateTime.GetLastUpdateTime());
	Send_IRCA(ChanOrPM(CD), "\00310Jugadores Online: \xF" + (std::string)playersNum + " | \00310Max desde ultimo reinicio: \xF" + (std::string)maxPlayersNum, true, CD->TYPE);
	Send_IRCA(ChanOrPM(CD), "\00310Conexiones activas: \xF" + (std::string)clientsNum + " | \00310Max desde ultimo reinicio: \xF" + (std::string)maxClientsNum, true, CD->TYPE);
	Send_IRCA(ChanOrPM(CD), "\00310Server: \xF" + svnrev + " |\00310 Update Time: \xF" + uptime + " |\00310 Diff: \xF" + updateTime, true, CD->TYPE);
...
void IRCCmd::Player_Player(_CDATA *CD)
{
	...
	uint32	account = 0;
	int32  gmlevel = 0;
	QueryResult result = CharacterDatabase.PQuery("SELECT guid,account FROM characters WHERE upper(name) = upper('%s')", _PARAMS[0].c_str());
	if (result)
	{
		Field *fields = result->Fetch();
		plguid = fields[0].GetUInt32();
		account = fields[1].GetUInt32();


		QueryResult result2 = LoginDatabase.PQuery("SELECT `gmlevel` FROM `account_access` WHERE id = %i",account);

		if (result2)
		{
			Field *fields2 = result2->Fetch();
			gmlevel = fields2[0].GetInt32();
		}
	}
	if (gmlevel > GetLevel(CD->USER) && (sIRC->BOTMASK & 512) != 0)
	{
		Send_IRCA(CD->USER, MakeMsg("No tienes acceso para esto con un GM[%i],es de mayor rango ", gmlevel), true, "ERROR");
		return;
	}
	...
	if (plguid > 0)
	{
		result = CharacterDatabase.PQuery("SELECT guid, account, name, race, class, online, level,  xp, money, totalHonorPoints, totaltime FROM characters WHERE guid =%i", plguid);
		uint32 latency = 0;
		...
		if (result)
		{
			...
			if (ponline == "\x3\x30\x33Online")
			{
				...
				if (plr)
				{
					...
					ponline += " en " + (std::string) area->area_name[6];
					if (area->zone != 0)
					{
						AreaTableEntry const* zone = sAreaTableStore.LookupEntry(area->zone);
						ponline += " (" + (std::string)zone->area_name[6] + ")";
					}
				}
			}
            #pragma warning(suppress: 4129)
			std::string pinfo = "\Sobre el jugador: \xF" + pname + " |\00310 GM Level: \xF" + pgmlvl + " |\00310 AcctID: \xF" + pacct + " |\00310 CharID: \xF" + pguid + " |\00310 Tiempo Jugador: \xF" + totaltim.c_str() + " |\00310 Latencia: \xF" + templatency;
			std::string pinfo2 = "\00310Raza: \xF" + (std::string)GetRaceName(praceid, 7) + " |\00310 Clase: \xF" + (std::string)GetClassName(pclassid, 6) + " |\00310 Nivel: \xF" + plevel + " |\00310 Dinero: \xF" + tempgold + "|\00310 Status: \xF" + ponline + " |\00310 Honor: \xF" + honor;
	...
	{
		QueryResult result = CharacterDatabase.PQuery("SELECT guid, account, name FROM characters WHERE upper(name) LIKE upper('%%%s%%') LIMIT 50", _PARAMS[0].c_str());
		if (result)
		{
			Field *fields = result->Fetch();
			std::string items = "\x2 Ningun personaje coincide con el nombre, estos son los posibles personajes:\x3\x31\x30 ";
			for (uint64 i = 0; i < result->GetRowCount(); i++)
			{
...
void IRCCmd::Lookup_Player(_CDATA *CD)
{
	...
	if (_PARAMS[0] == "char")
	{
		...
		if (ObjectAccessor::FindPlayerByName(_PARAMS[1].c_str()))
			plguid = ObjectAccessor::FindPlayerByName(_PARAMS[1].c_str())->GetGUID();
		if (plguid > 0)
		{
...
void IRCCmd::Level_Player(_CDATA *CD)
{
	...
	ObjectGuid guid = ObjectAccessor::FindPlayerByName(player.c_str())->GetGUID();
	...
	else
	{
		...
		int32 i_oldlvl = chr ? chr->getLevel() : 1;
...
void IRCCmd::Online_Players(_CDATA CD)
{
	...
	boost::thread script([_CD]()
	{
		mcs_OnlinePlayers mcs(_CD);
...
void IRCCmd::Tele_Player(_CDATA *CD)
{
	...
	else if (_PARAMS[1] == "to")
	{
		...
		{
			if (ObjectGuid guid = ObjectAccessor::FindPlayerByName(_PARAMS[2].c_str())->GetGUID())
			{
				bool in_flight;
