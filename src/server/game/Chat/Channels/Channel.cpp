    ...
    if (channelId)
    {
        ...
        if (!(channelEntry->flags & CHANNEL_DBC_FLAG_GLOBAL))
        {
            if (channelEntry->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                channelName = Trinity::StringFormat(channelEntry->pattern[6], sObjectMgr->GetTrinityString(LANG_CHANNEL_CITY, LOCALE_esES));
            else
                channelName = Trinity::StringFormat(channelEntry->pattern[6], ASSERT_NOTNULL(zoneEntry)->area_name[6]);
        }
        else
            channelName = channelEntry->pattern[6];
    }
}
...
std::string Channel::GetName(LocaleConstant locale /*= DEFAULT_LOCALE*/) const
{
    ...
    Channel::GetChannelName(result, _channelId, LOCALE_esES, _zoneEntry);
...
void Channel::JoinChannel(Player* player, std::string const& pass)
{
	...
	sIRC->Handle_WoW_Channel(this->GetName(), ObjectAccessor::FindPlayer(guid), CHANNEL_JOIN);
...
void Channel::LeaveChannel(Player* player, bool send)
{
	...
	sIRC->Handle_WoW_Channel(this->GetName(), ObjectAccessor::FindPlayer(guid), CHANNEL_LEAVE);
    LeaveNotify(guid);
...
void Channel::List(Player const* player) const
{
    ...
    std::string channelName = GetName(LOCALE_esES);
...
void Channel::Say(ObjectGuid guid, std::string const& what, uint32 lang) const
{
    ...
    {
        LocaleConstant localeIdx = sWorld->GetAvailableDbcLocale(LOCALE_esES);
