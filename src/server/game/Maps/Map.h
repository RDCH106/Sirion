...
private:
    ...
    NGridType* getNGrid(uint32 x, uint32 y) const
    {
        ...
        uint32 gx = x < MAX_NUMBER_OF_GRIDS - 1 ? x : MAX_NUMBER_OF_GRIDS - 1;
        uint32 gy = y < MAX_NUMBER_OF_GRIDS - 1 ? y : MAX_NUMBER_OF_GRIDS - 1;

        ASSERT(gx < MAX_NUMBER_OF_GRIDS && gy < MAX_NUMBER_OF_GRIDS);
        return i_grids[gx][gy];
    }
