...
void Map::EnsureGridCreated_i(GridCoord const& p)
{
	if (!getNGrid(p.x_coord, p.y_coord))
	{
		...
		//z coord
		uint32 gx = (MAX_NUMBER_OF_GRIDS - 1) - p.x_coord;
		uint32 gy = (MAX_NUMBER_OF_GRIDS - 1) - p.y_coord;

		//Prevengo que si esta fuera del grid, cause ASSERT al corregir a la ultima celda del grid
		gx = gx < MAX_NUMBER_OF_GRIDS - 1 ? gx : MAX_NUMBER_OF_GRIDS - 1;
		gy = gy < MAX_NUMBER_OF_GRIDS - 1 ? gy : MAX_NUMBER_OF_GRIDS - 1;
...
inline GridMap* Map::GetGrid(float x, float y)
{
	...
	if (gx >= MAX_NUMBER_OF_GRIDS)
		gx = MAX_NUMBER_OF_GRIDS - 1;
	if (gy >= MAX_NUMBER_OF_GRIDS)
		gy = MAX_NUMBER_OF_GRIDS - 1;
...
Map::EnterState InstanceMap::CannotEnter(Player* player)
{
	...
	//Dado que hay guild que se aprobechan de esto para meter a sus alter cuando su main muere, y continuar el boss con mas pj de los que deberian entrar,
	//impido que se entre aunque se este logueando, de modo que no se pueda exploitear
	//para hacer una raid de 10 con mas de 10 pj o de 25 con mas de 25
