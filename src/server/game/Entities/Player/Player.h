...
public:
    ...
    // PlayedTimeReward
    uint32 ptr_Money_Interval;
    uint32 ptr_Honor_Interval;
    uint32 ptr_Arena_Interval;
    uint32 ptr_Loyalty_Interval;

    //Fix Chamanes Guardian de la Naturaleza
    bool AllowCastGuardian() { return m_NextCastGuardian < GameTime::GetGameTimeSystemPoint(); }
    void SetNextCastGuardian(Milliseconds next) { m_NextCastGuardian = GameTime::GetGameTimeSystemPoint() + next; }
    ...
    void DestroyItem(uint8 bag, uint8 slot, bool update,bool log = true);
  ...
  //void SetSelection(ObjectGuid guid) { SetGuidValue(UNIT_FIELD_TARGET, guid); }
    ...
    //GOSSIP HELLO REQUEST
    void SendGossipRequest(uint32 gossipID);
    void SetCurrentNPCGossip(uint32 gossipID) { m_currentNPCGossip = gossipID; }
    bool IsCurrentNPCGossip(uint32 gossipID) { return m_currentNPCGossip == gossipID; }

    void SetCurrentItemGossip(uint32 gossipID) { m_currentItemGossip = gossipID; }
    bool IsCurrentItemGossip(uint32 gossipID) { return m_currentItemGossip == gossipID; }

    //LOYALTY SHOP
    void SetLoyaltyShopParameters(uint32 category, uint32 offset, uint32 limit) { m_LoyaltyShop_category = category; m_LoyaltyShop_offset = offset; m_LoyaltyShop_limit = limit; }
    uint32 GetLoyaltyShopCategory() { return m_LoyaltyShop_category; }
    uint32 GetLoyaltyShopOffset() { return m_LoyaltyShop_offset; }
    uint32 GetLoyaltyShopLimit() { return m_LoyaltyShop_limit; }

    //Enviar item Shop
    bool SendShopItem(std::vector<RewardsMenuTemplate> rewards) { return _EnviarPorCorreo(rewards); }

    std::string GetMapAreaAndZoneString() const;
    std::string GetCoordsMapAreaAndZoneString() const;
    TransmogMapType transmogMap; // transmogMap[iGUID] = entry

    void SetTmorgOffset(uint32 offset) { m_TmorgOffset = offset; }
    uint32 GetTmorgOffset() { return m_TmorgOffset; }
    void SetTmorgCategory(uint32 Category) { m_TmorgCategory = Category; }
    uint32 GetTmorgCategory() { return m_TmorgCategory; }
    void SetTmorgOnlyMyClass(bool OnlyMyClass) { m_TmorgOnlyMyClass = OnlyMyClass; }
    bool GetTmorgOnlyMyClass() { return m_TmorgOnlyMyClass; }

    ReforgeMapType reforgeMap; // reforgeMap[iGUID] = ReforgeData

    bool GetHuarguenState() { return m_HuargenState; }
    void SetHuargenState(bool state);
    //PromoGuild
    bool HasPromoDebuff();
    ...
    bool IsLevelAnnounceActiveFor() { return m_bLevelAnnounceActive; }
...
protected:
    ...
    //LoyaltyShop
    uint32 m_LoyaltyShop_category = 0;
    ...
    void _AddTmorgItem(const ItemTemplate* proto);

    void _GetPromocionGuild();


    uint32 m_TmorgOffset = 0;
    uint32 m_TmorgCategory = 0;
    bool m_TmorgOnlyMyClass = true;
    /*********************************************************/
    /***               BATTLEGROUND SYSTEM                 ***/
    /*********************************************************/
...
private:
    ...
    //Variable para el fix del talento del chaman Guardian de la Naturaleza
    std::chrono::system_clock::time_point m_NextCastGuardian;
