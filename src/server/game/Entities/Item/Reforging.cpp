...
void SendReforgePacket(Player* player, uint32 entry, uint32 lowguid, const ReforgeData* reforge)
{
	...
	LocaleConstant loc_const = player->GetSession()->GetSessionDbcLocale();
	if (loc_const)// loc_idx >= 0)
	{
		if (ItemLocale const* il = sObjectMgr->GetItemLocale(pProto->ItemId))
		{
			ObjectMgr::GetLocaleString(il->Name, loc_const, Name);
			ObjectMgr::GetLocaleString(il->Description, loc_const, Description);
