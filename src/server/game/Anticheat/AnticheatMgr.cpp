...
void AnticheatMgr::ExternalProgramDetection(Player* player)
{
    ...
    TC_LOG_DEBUG("entities.player.character", "AnticheatMgr:: External Program detected player GUID (low) %u", player->GetGUID().GetCounter());

    BuildReport(player, EXTERNAL_PROGRAM_REPORT);
}
...
void AnticheatMgr::SpeedHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    uint32 speedRate = (uint32)((player->GetSpeed(UnitMoveType(moveType)) + movementInfo.jump.xyspeed) * sWorld->getFloatConfig(CONFIG_ANTICHEAT_SPEED_fACTOR)); //Turrado, añado el *1.25 para dar margen de pasarse un poco
...
void AnticheatMgr::SavePlayerData(Player* player, SQLTransaction& trans)
{
    // prepare Anticheat datas
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_ANTICHEAT_STATUS);
    stmt->setUInt32(0, player->GetGUID().GetCounter());
    stmt->setUInt32(1, m_Players[player->GetGUID().GetCounter()].GetAverage());
    stmt->setUInt32(2, m_Players[player->GetGUID().GetCounter()].GetTotalReports());
    stmt->setUInt32(3, m_Players[player->GetGUID().GetCounter()].GetTypeReports(SPEED_HACK_REPORT));
    stmt->setUInt32(4, m_Players[player->GetGUID().GetCounter()].GetTypeReports(FLY_HACK_REPORT));
    stmt->setUInt32(5, m_Players[player->GetGUID().GetCounter()].GetTypeReports(JUMP_HACK_REPORT));
    stmt->setUInt32(6, m_Players[player->GetGUID().GetCounter()].GetTypeReports(WALK_WATER_HACK_REPORT));
    stmt->setUInt32(7, m_Players[player->GetGUID().GetCounter()].GetTypeReports(TELEPORT_PLANE_HACK_REPORT));
    stmt->setUInt32(8, m_Players[player->GetGUID().GetCounter()].GetTypeReports(CLIMB_HACK_REPORT));
    stmt->setUInt32(9, m_Players[player->GetGUID().GetCounter()].GetTypeReports(EXTERNAL_PROGRAM_REPORT));
    stmt->setUInt32(10, m_Players[player->GetGUID().GetCounter()].GetCreationTime());    

    // add to Anticheat
    trans->Append(stmt);


    //CharacterDatabase.PExecute("REPLACE INTO players_reports_status (guid,average,total_reports,speed_reports,fly_reports,jump_reports,waterwalk_reports,teleportplane_reports,climb_reports,external_reports,creation_time) VALUES (%u,%f,%u,%u,%u,%u,%u,%u,%u,%u,%u);", player->GetGUID().GetCounter(), m_Players[player->GetGUID().GetCounter()].GetAverage(), m_Players[player->GetGUID().GetCounter()].GetTotalReports(), m_Players[player->GetGUID().GetCounter()].GetTypeReports(SPEED_HACK_REPORT), m_Players[player->GetGUID().GetCounter()].GetTypeReports(FLY_HACK_REPORT), m_Players[player->GetGUID().GetCounter()].GetTypeReports(JUMP_HACK_REPORT), m_Players[player->GetGUID().GetCounter()].GetTypeReports(WALK_WATER_HACK_REPORT), m_Players[player->GetGUID().GetCounter()].GetTypeReports(TELEPORT_PLANE_HACK_REPORT), m_Players[player->GetGUID().GetCounter()].GetTypeReports(CLIMB_HACK_REPORT), m_Players[player->GetGUID().GetCounter()].GetTypeReports(EXTERNAL_PROGRAM_REPORT), m_Players[player->GetGUID().GetCounter()].GetCreationTime());
}
...
void AnticheatMgr::BuildReport(Player* player, uint8 reportType)
{
    ...
    if (sWorld->getIntConfig(CONFIG_ANTICHEAT_MAX_REPORTS_FOR_DAILY_REPORT) < m_Players[key].GetTotalReports())
    {
        ...
        {
            // prepare Anticheat datas
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_ANTICHEAT_DAILY);
            stmt->setUInt32(0, player->GetGUID().GetCounter());
            stmt->setUInt32(1, m_Players[player->GetGUID().GetCounter()].GetAverage());
            stmt->setUInt32(2, m_Players[player->GetGUID().GetCounter()].GetTotalReports());
            stmt->setUInt32(3, m_Players[player->GetGUID().GetCounter()].GetTypeReports(SPEED_HACK_REPORT));
    ...
    if (m_Players[key].GetTotalReports() > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
    {
        ...
        std::string str = "";
        str = "|cFFFFFC00[AC]|cFF00FFFF[|cFF60FF00" + std::string(player->GetName().c_str()) + "|cffff0000] Posible hacker!";
        ...
        //player->TeleportTo(1, 16226.5f, 16403.6f, -64.5f, 3.2f);
    }
}
...
void AnticheatMgr::AnticheatGlobalCommand(ChatHandler* handler)
{
    ...
    else
    {
        ...
        handler->PSendSysMessage("Los jugadores con los promedios mas bajos:");
        do
        {
            ...
            if (Player* player = ObjectAccessor::FindPlayerByLowGUID(guid))
                handler->PSendSysMessage("Jugador: %s Promerdio: %f Reportes Totales: %u", player->GetName().c_str(), average, total_reports);
    ...
    {
        handler->SendSysMessage("=============================");
        handler->PSendSysMessage("Jugadores con mayor cantidad de reportes:");
        do
        {
            Field *fieldsDB = resultDB->Fetch();
            ...
            if (Player* player = ObjectAccessor::FindPlayerByLowGUID(guid))
                handler->PSendSysMessage("Jugador: %s Total Reportes Totales: %u Promerdio: %f", player->GetName().c_str(), total_reports, average);
