...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    // DeserterTracker
    PrepareStatement(CHAR_INS_DESERTER_TRACK, "INSERT INTO battleground_deserters (guid, type, datetime) VALUES (?, ?, NOW())", CONNECTION_BOTH);
    ...
    PrepareStatement(CHAR_SEL_GUILD_TOP, "SELECT level,name FROM guild where level > 0 or xp > 0 order by level desc, xp desc limit ?", CONNECTION_SYNCH);
    ...
    //Anticheat
    PrepareStatement(CHAR_REP_ANTICHEAT_DAILY, "REPLACE INTO daily_players_reports (guid,average,total_reports,speed_reports,fly_reports,jump_reports,waterwalk_reports,teleportplane_reports,climb_reports,external_reports,creation_time) VALUES (?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_ANTICHEAT_STATUS, "REPLACE INTO players_reports_status (guid,average,total_reports,speed_reports,fly_reports,jump_reports,waterwalk_reports,teleportplane_reports,climb_reports,external_reports,creation_time) VALUES (?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    //Mount Save
    PrepareStatement(CHAR_REP_MOUNT, "REPLACE INTO character_mount (guid,spell) VALUES (?,?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_MOUNT, "SELECT spell from character_mount where guid = ?", CONNECTION_SYNCH);
	//NEWS SYSTEM
	PrepareStatement(CHAR_UPD_RESET_POPUP, "Update characters set popup = 0", CONNECTION_ASYNC);
	PrepareStatement(CHAR_UPD_SET_POPUP, "Update characters set popup = 1 where guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_SEL_POPUP, "Select popup from characters where guid = ?", CONNECTION_SYNCH);
