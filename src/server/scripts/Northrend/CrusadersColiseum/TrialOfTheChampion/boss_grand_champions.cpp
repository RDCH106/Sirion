...
struct boss_grand_championAI : BossAI
{
    ...
    {
        //Cambio el sistema para que retorne si TODOs estan fuera de sus monturas, no que uno lo este
        for (int i = 0; i < 3; ++i)
        {
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage) override
    {
        if (damage >= me->GetHealth() && me->GetVehicleBase())
        {
            ...
            if (Creature* mount = FindMyMount(me))
            {
                ...
                Cell::VisitAllObjects(me, searcher, me->GetGridActivationRange());
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (instance->GetBossState(DATA_GRAND_CHAMPIONS) == IN_PROGRESS && !MountedPhaseDone && GrandChampionsOutOfVehicles())
        {
                                ...
                                if (rand == 0)
                                {
                                    ...
                                    if (plr && !plr->IsGameMaster() && plr->IsAlive() && me->IsInRange(plr, 8.0f, 25.0f, false))
                                    {
                                            ...
                                            if (me->GetVehicleKit() && me->GetVehicleKit()->GetPassenger(SEAT_ID_0))
                                            {
                                                if (Creature* rider = me->GetVehicleKit()->GetPassenger(SEAT_ID_0)->ToCreature())
                                                {
                                                    ...
                                                    if (plr->GetVehicleBase())
                                                        AddThreat(plr->GetVehicleBase(), 100000.0f, rider);
                                                    else
                                                        AddThreat(plr, 100000.0f, rider);
                                        ...
                                        else
                                        {
                                            ...
                                            // Setting gaze on the new player
                                            if (plr->GetVehicleBase())
                                                AddThreat(plr->GetVehicleBase(), 100000.0f, me);
                                            else
                                                AddThreat(plr, 100000.0f, me);
                                            // Casting actual charge
                                            if (plr->GetVehicleBase())
    ...
    class spell_toc5_trample_aura_SpellScript : public SpellScript
    {
        ...
        void HandleStun()
        {
            if (Unit* target = GetHitUnit())
            {
                ...
                auto DebugCaster = caster->GetName();
                auto MonturaCasterPlayer = caster->GetVehicleKit();// Players
                auto MonturaCasterBoss = caster->GetVehicleCreatureBase(); // Grand Champions
                auto MonturaCasterLesser = caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_MOUNT); // Lesser Champions

                auto DebugTarget = target->GetName();
                auto MonturaPlayer = target->GetVehicleKit();// Players
                auto MonturaBoss = target->GetVehicleCreatureBase(); // Grand Champions
                auto MonturaLesser = target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_MOUNT); // Lesser Champions

                //Si el caster no esta montado,
                //ó el player esta montado
                //ó el boss esta montado
                //ó el add esta montado
                //Skipeamos el efecto del aura
                if ((!MonturaCasterPlayer && !MonturaCasterBoss && !MonturaCasterLesser) || MonturaPlayer || MonturaBoss || MonturaLesser)
                {
