...
public:
    ...
    struct instance_trial_of_the_champion_InstanceMapScript : public InstanceScript
    {
        ...
        {
            player->RemoveAura(AuraStunJusta);
        }       
        ...
        bool SetBossState(uint32 id, EncounterState state) override
        {
            ...
            //Condiciones para seteos en la justa
            if (id == DATA_GRAND_CHAMPIONS)
            {
                //Antes de setearlo en DONE, compruebo si estamos en proceso del primer boss, ya que se marca como DONE al volver de un wipe
                //Antes de setearlo en NOT_STARTED , compruebo si estamos en proceso del primer boss, ya que se marca como NOT_STARTED al matar al primer boss
                //porque lo manda a evasion
                if ((GetBossState(DATA_GRAND_CHAMPIONS) != IN_PROGRESS && state == DONE)
                    || (GetBossState(DATA_GRAND_CHAMPIONS) == IN_PROGRESS && state == NOT_STARTED))
                    return false;
            }
            ...
            case DATA_GRAND_CHAMPIONS:
                ...
                else if (state == FAIL)
                {
                    for (uint8 i = 0; i < GrandChampionsCount; ++i)
                    {
                        _grandChampionGUIDs[i] = ObjectGuid::Empty;
                    }
                }
                break;
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            case DATA_REMOVE_VEHICLES:
            {
                ...
                //Quito el aura de stun a los players
                DoRemoveAurasDueToSpellOnPlayers(AuraStunJusta);
