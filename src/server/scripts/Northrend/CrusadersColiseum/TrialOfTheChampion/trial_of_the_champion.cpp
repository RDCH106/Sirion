...
#define GOSSIP_START_EVENT1     "Estoy listo."
#define GOSSIP_START_EVENT2     "Estoy listo para el siguiente desafio."
#define GOSSIP_START_EVENT_SKIP "Estoy listo, saltemonos las presentaciones."
...
public:
    ...
    struct npc_announcer_toc5AI : public ScriptedAI
    {
        ...
        void DoSummonNextGrandChampion(bool skipEvent = false)
        {
            ...
            if (Creature* pBoss = me->SummonCreature(info->GrandChampionEntry[teamIndex], spawnPos))
            {
                ...
                if (!skipEvent)
                {
                    ...
                    //Los muevo hacia su posicion
                    Position  finalPos = GrandChampFinalLoc[x];
                    finalPos.SetOrientation(TeamInInstance == ALLIANCE? hordeOrientation: allianceOrientation);
                    //Mando un 2 ya que el 0 busca montura y el 1 despawnead
                    pBoss->GetMotionMaster()->MovePoint(2, finalPos);
                }
                else
                {
        ...
        void ResetEncounter()
        {
            //Lo mando a home, y le marco que puede hablarse con el          
            //me->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
            me->GetMotionMaster()->MovePoint(POINT_RETURN_TO_CENTER, me->GetHomePosition());
            ...
            for (int i = 0; i < 3; ++i)
            {
                if (Creature* pGrandChampion = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_GRAND_CHAMPION_1 + i)))
                {
                    if (Creature* mount = pGrandChampion->GetVehicleCreatureBase())
                    {
                        mount->DespawnOrUnsummon();
                    }                   
                    pGrandChampion->DespawnOrUnsummon();
                }
            }
            

            uiSummonTimes = 0;
            instance->SetBossState(DATA_GRAND_CHAMPIONS, FAIL);
        }
        ...
        void StartEncounter(bool startRp = false)
        {
            ...
            if (instance->GetBossState(DATA_BLACK_KNIGHT) == NOT_STARTED || instance->GetBossState(DATA_BLACK_KNIGHT) == TO_BE_DECIDED)
            {
                if (instance->GetBossState(DATA_ARGENT_CHALLENGE) == NOT_STARTED || instance->GetBossState(DATA_ARGENT_CHALLENGE) == TO_BE_DECIDED)
                {
                    ...
                    if (startRp && (instance->GetBossState(DATA_GRAND_CHAMPIONS) == NOT_STARTED || instance->GetBossState(DATA_GRAND_CHAMPIONS) == FAIL || instance->GetBossState(DATA_GRAND_CHAMPIONS) == TO_BE_DECIDED))
                    {
                        Talk(SAY_INTRO_1);
        ...
        bool GossipHello(Player* player) override
        {
            ...
            if (instance)
            {
                ...
                if (player->IsGameMaster())
                {
                    ...
                    if ((instance->GetBossState(DATA_GRAND_CHAMPIONS) == NOT_STARTED || instance->GetBossState(DATA_GRAND_CHAMPIONS) == FAIL || instance->GetBossState(DATA_GRAND_CHAMPIONS) == TO_BE_DECIDED) &&
                    ...
                    {
                        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "[GM] Empezar grandes campeones,con intro", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
                        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "[GM] Empezar grandes campeones,sin intro", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                    }
                    if (instance->GetBossState(DATA_ARGENT_CHALLENGE) == NOT_STARTED || instance->GetBossState(DATA_ARGENT_CHALLENGE) == TO_BE_DECIDED)
                    {
                        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "[GM] Empezar Eadric Boss2A", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
                        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "[GM] Empezar Paletress Boss2B", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);
                    }
                    if (instance->GetBossState(DATA_BLACK_KNIGHT) == NOT_STARTED || instance->GetBossState(DATA_BLACK_KNIGHT) == TO_BE_DECIDED)
                        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "[GM] Empezar caballero negro", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5);
                    SendGossipMenuFor(player, 1, me->GetGUID());
                }
                else if ((instance->GetBossState(DATA_GRAND_CHAMPIONS) == NOT_STARTED || instance->GetBossState(DATA_GRAND_CHAMPIONS) == FAIL || instance->GetBossState(DATA_GRAND_CHAMPIONS) == TO_BE_DECIDED) && player->GetVehicleBase())
                {
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (!me->SelectNearestPlayer(250))
            {
                //Compruebo el estado del primer boss 
                if (instance->GetBossState(DATA_GRAND_CHAMPIONS) == IN_PROGRESS)
                {
                    ResetEncounter();                   
                }
            }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CHAT_7:
                    ...
                    break;
                case EVENT_AGGRO_1:
                    //Cuando se ponen agresivo le pongo directamente en progreso al boss
                    instance->SetBossState(DATA_GRAND_CHAMPIONS, IN_PROGRESS);
                    //Doy el aura de stun a los players
                    instance->DoCastSpellOnPlayers(67870);                    
