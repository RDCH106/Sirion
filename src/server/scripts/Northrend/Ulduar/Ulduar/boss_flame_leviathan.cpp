            ...
            void DoAction(int32 action) override
            {
                ...
                if (action == ACTION_SPAWN_VEHICLES)
                {
                    ...
                    for (uint8 i = 0; i < RAID_MODE(2, 5); ++i)
                        _instance->OnCreatureCreate(DoSummon(VEHICLE_SIEGE, PosSiege[i], 3000, TEMPSUMMON_CORPSE_TIMED_DESPAWN));
                    for (uint8 i = 0; i < RAID_MODE(2, 5); ++i)
                        _instance->OnCreatureCreate(DoSummon(VEHICLE_CHOPPER, PosChopper[i], 3000, TEMPSUMMON_CORPSE_TIMED_DESPAWN));
                    for (uint8 i = 0; i < RAID_MODE(2, 5); ++i)
                        _instance->OnCreatureCreate(DoSummon(VEHICLE_DEMOLISHER, PosDemolisher[i], 3000, TEMPSUMMON_CORPSE_TIMED_DESPAWN));
                }
            }
