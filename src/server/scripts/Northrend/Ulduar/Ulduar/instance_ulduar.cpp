    ...
    public:
        ...
        struct instance_ulduar_InstanceMapScript : public InstanceScript
        {
            ...
            void OnCreatureCreate(Creature* creature) override
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_SALVAGED_CHOPPER:
                        //Como cuando respawnean no se vigila, compruebo al crearla si leviatan ya ha muerto, y lo deshabilito
			...
			bool CheckRequiredBosses(uint32 bossId, Player const* player = nullptr) const override
			{
                ...
                {
                case BOSS_ASSEMBLY_OF_IRON:
                    //Compruebo para evitar a la gente que se salta los bosses con hack
                    if (GetBossState(0) != DONE || GetBossState(1) != DONE || GetBossState(2) != DONE || GetBossState(3) != DONE)
                    {
                        return false;
                    }                    
                    break;
