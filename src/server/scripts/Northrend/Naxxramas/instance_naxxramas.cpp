    ...
    struct instance_naxxramas_InstanceMapScript : public InstanceScript
    {
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            ...
            case GO_BIRTH:
                ...
                break;
            case DATA_NAXX_ORB://Pongo el orbe en no selectable
                ModOrbSelectable();
            default:
                break;
            }
        ...
        void ModOrbSelectable()
        {
            if (GetBossState(BOSS_THADDIUS) == DONE && GetBossState(BOSS_HORSEMEN) == DONE && GetBossState(BOSS_LOATHEB) == DONE && GetBossState(BOSS_MAEXXNA) == DONE)
            {
                if (GameObject* go_Orb = GetGameObject(DATA_NAXX_ORB))
                    go_Orb->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
            }
            else
            {
                if (GameObject* go_Orb = GetGameObject(DATA_NAXX_ORB))
                    go_Orb->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
            }
