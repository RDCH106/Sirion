...
public:
	...
	{
		bool m_bSpellDone = false;
		bool m_bFactionSetted = false;
		uint32 m_nTimmerToSetFaction = 1000;
		...
		{
			UpdateVictim();
			if (m_bSpellDone && !m_bFactionSetted)
			{
				if (m_nTimmerToSetFaction <= diff)
				{
					me->SetFaction(168);
					m_bFactionSetted = true;
				}
				else m_nTimmerToSetFaction -= diff;
			}
		}
