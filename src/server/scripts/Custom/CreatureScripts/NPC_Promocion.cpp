						...
						if (hermandad->GetLeaderGUID() == player->GetGUID())
						{
							if (hermandad->GetMemberCount() >= 15)
							{
								...
								if (BankTabCount < 6)
								{
									...
									for (int i = 0; i < 6; i++) hermandad->HandleBuyBankTab(player->GetSession(), i);
									player->GetSession()->SendAreaTriggerMessage("Promocion aplicada");
								}
								else player->GetSession()->SendNotification("La promocion ya estaba aplicada");

							}
							else player->GetSession()->SendNotification("La hermandad no tiene suficiente gente");
						}
...
#pragma endregion
			...
			case PROFESIONES:
				...
				default:
					...
					player->SetSkill(action, player->GetSkillStep(action), maxLevel, maxLevel);
			...
			case PVE:
				...
				case PALA_DPS:
					...
					else if (player->GetTeamId() == 7)player->AddItem(45074, 1);
