...
class spell_gen_aura_dos_formas : public AuraScript
{
	PrepareAuraScript(spell_gen_aura_dos_formas);

	bool Validate(SpellInfo const* /*spellInfo*/) override
	{
		if (!sSpellMgr->GetSpellInfo(87840))
			return true;
	}
	...
	void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
	{
		...
		if (target->GetTypeId() == TYPEID_PLAYER)
		{
			target->RemoveAura(68996);

			if (target->ToPlayer()->GetHuarguenState()) //Aura de Huargen
			{
				target->RemoveAurasByType(SPELL_AURA_TRANSFORM);
				if (target->getGender() == GENDER_MALE)
				{
					target->SetDisplayId(2252);// 19723
					target->SetNativeDisplayId(2252);// 19723
				}
				else
				{
					target->SetDisplayId(9339);//19724
					target->SetNativeDisplayId(9339);//19724
				}
				target->ToPlayer()->SetHuargenState(false);
			}
			else
			{
				target->RemoveAurasByType(SPELL_AURA_TRANSFORM);
				if (target->getGender() == GENDER_MALE)
				{
					target->SetNativeDisplayId(2243); //202
					target->RestoreDisplayId();
				}
				else
				{
					target->SetNativeDisplayId(2231);  //203
					target->RestoreDisplayId();
				}
				target->ToPlayer()->SetHuargenState(true);
	...
	void Register() override
	{
		AfterEffectApply += AuraEffectRemoveFn(spell_gen_aura_dos_formas::OnApply, EFFECT_0, SPELL_AURA_TRANSFORM, AURA_EFFECT_HANDLE_REAL);
	}

};

class spell_gen_aura_conversion_lobo : public AuraScript
{
	PrepareAuraScript(spell_gen_aura_conversion_lobo);
	...
	void Register() override
	{
		AfterEffectApply += AuraEffectRemoveFn(spell_gen_aura_conversion_lobo::OnApply, EFFECT_0, SPELL_AURA_TRANSFORM, AURA_EFFECT_HANDLE_REAL);
	}

};


enum GalopadaSalvajeMountIds
{
...
class spell_gen_aura_galopada_salvaje : public AuraScript
{
	PrepareAuraScript(spell_gen_aura_galopada_salvaje);
	...
	void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
	{
		...
		{			
			...
			target->ToPlayer()->Mount(23509);
			uint32 mount = 0;
			switch (target->ToPlayer()->GetBaseSkillValue(SKILL_RIDING))
			{
			case 0:
				mount = 0;
				break;
			...
			target->AddAura(mount, target);

			target->ToPlayer()->AddAura(68995, target);
	...
	void Register() override
	{
		OnEffectApply += AuraEffectRemoveFn(spell_gen_aura_galopada_salvaje::OnApply, EFFECT_0, SPELL_AURA_MOUNTED, AURA_EFFECT_HANDLE_REAL);
	}
};


class spell_gen_aura_huida_oscura : public SpellScriptLoader
{
public:
	spell_gen_aura_huida_oscura() : SpellScriptLoader("spell_gen_aura_huida_oscura") { }

	class spell_gen_aura_huida_oscura_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_gen_aura_huida_oscura_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(68992))
				return true;
		}
		...
		void Register() override
		{
			AfterEffectApply += AuraEffectRemoveFn(spell_gen_aura_huida_oscura_AuraScript::OnApply, EFFECT_0, SPELL_AURA_TRANSFORM, AURA_EFFECT_HANDLE_REAL);
			AfterEffectRemove += AuraEffectRemoveFn(spell_gen_aura_huida_oscura_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_TRANSFORM, AURA_EFFECT_HANDLE_REAL);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_gen_aura_huida_oscura_AuraScript();
	}
};
...
enum Mounts
{
	...
	// Montura Terrestre importante
	SPELL_MONTURA_TERRESTRE_60 = 6000314,
	SPELL_MONTURA_TERRESTRE_100 = 6000315,
	SPELL_MONTURA_TERRESTRE_150 = 6000321,
	SPELL_MONTURA_TERRESTRE_280 = 6000322,
	SPELL_MONTURA_TERRESTRE_310 = 6000323,

	// Montura Voladora importante
	SPELL_MONTURA_VOLADORA_60 = 6000316,
	SPELL_MONTURA_VOLADORA_100 = 6000317,
	SPELL_MONTURA_VOLADORA_150 = 6000318,
	SPELL_MONTURA_VOLADORA_280 = 6000319,
	SPELL_MONTURA_VOLADORA_310 = 6000320
};
...
void AddSC_generic_spell_scripts()
{
	...
	RegisterAuraScript(spell_gen_aura_dos_formas);
	RegisterAuraScript(spell_gen_aura_conversion_lobo);
	RegisterAuraScript(spell_gen_aura_galopada_salvaje);
	new spell_gen_aura_huida_oscura();
	...
	new spell_gen_mount("spell_montura_terrestre", 0, SPELL_MONTURA_TERRESTRE_60, SPELL_MONTURA_TERRESTRE_100, SPELL_MONTURA_TERRESTRE_150, SPELL_MONTURA_TERRESTRE_280, SPELL_MONTURA_TERRESTRE_310);
	new spell_gen_mount("spell_montura_voladora", 0, SPELL_MONTURA_VOLADORA_60, SPELL_MONTURA_VOLADORA_100, SPELL_MONTURA_VOLADORA_150, SPELL_MONTURA_VOLADORA_280, SPELL_MONTURA_VOLADORA_310);
