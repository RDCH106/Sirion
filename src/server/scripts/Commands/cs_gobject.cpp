...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> gobjectCommandTable =
        {
            ...
            { "ActivateLoot",   rbac::RBAC_PERM_COMMAND_GOBJECT_ACTIVE_LOOT,  false, &HandleGameObjectActiveForLootCommand,  "" },
        };
    ...
    static bool HandleGameObjectActiveForLootCommand(ChatHandler* handler, char const* args)
    {
        // number or [name] Shift-click form |color|Hgameobject:go_id|h[name]|h|r
        char* id = handler->extractKeyFromLink((char*)args, "Hgameobject");
        if (!id)
        ...
        if (!guidLow)
            return false;
       uint32 nguidLow = atoi(id);
        if (!nguidLow)
            return false;
      
        GameObject* object = handler->GetSession()->GetPlayer()->FindNearestGameObject(nguidLow,5);
        if (!object)
        {
            handler->PSendSysMessage(LANG_COMMAND_OBJNOTFOUND, guidLow);
            handler->SetSentErrorMessage(true);
        ...
        //Quito las flag que impiden loteo
        object->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_LOCKED | GO_FLAG_NOT_SELECTABLE | GO_FLAG_NODESPAWN);      
