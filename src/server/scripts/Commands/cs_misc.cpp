    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (area)
        {
            ...
            areaName = area->area_name[nDBC_Locale];

            AreaTableEntry const* zone = sAreaTableStore.LookupEntry(area->zone);
            if (zone)
                zoneName = zone->area_name[nDBC_Locale];
        }

        if (target)
            handler->PSendSysMessage(LANG_PINFO_CHR_MAP, map->name[nDBC_Locale],
    ...
    static bool HandleGossipRequest(ChatHandler* handler, char const* args)
    {
        ...
        int32 gossip_int = atoi((char*)gossip);
        if (gossip_int < 0)
            return false;
        if (gossip_int >= MAX_GOSSIP_HELLO_REQUEST)
            return false;

        handler->GetSession()->GetPlayer()->SendGossipRequest(gossip_int);
        return true;
    }
    ...
    static bool HandleLoyaltyShopRequest(ChatHandler* handler, char const* args)
    {
        ...
        int32 category_int = atoi((char*)category);
        if (category_int <= 0)
            return false;
        ...
        int32 offset_int = atoi((char*)offset);
        if (offset_int < 0)
            return false;
        ...
        int32 limit_int = atoi((char*)limit);
        if (limit_int <= 0)
            return false;

        handler->GetSession()->GetPlayer()->SetLoyaltyShopParameters(category_int, offset_int, limit_int);
        handler->GetSession()->GetPlayer()->SendGossipRequest(LOYALTY_SHOP_HELLO_REQUEST);
        return true;
    }

    static bool HandleSetHasPatch(ChatHandler* handler, char const* /* args*/)
    {
        Player* player = handler->GetSession()->GetPlayer();
        if (!player)
            return false;
