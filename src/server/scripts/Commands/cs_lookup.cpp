...
#define LocaleDBC_Rusa 8
#define LocaleDBC_Mex 7
#define LocaleDBC_Esp 6
...
public:
    ...
    static bool HandleLookupSkillCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
        {
            ...
            {
                int locale = handler->GetSessionDbcLocale();
                std::string name = skillInfo->name[LocaleDBC_Mex];
                if (name.empty())
                    continue;
    ...
    static bool HandleLookupSpellCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); ++id)
        {
            ...
            if (spellInfo)
            {
                ...
                std::string name = spellInfo->SpellName[LocaleDBC_Rusa];

                if (name.empty())
                    continue;

                //AL FIJAR LA DBC; no necesitamos iterar, si esta esta y si no esta no lo esta
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (handler->GetSession())
                        ss << ' ' << localeNames[LocaleDBC_Esp] << "]|h|r";
                    else
                        ss << ' ' << localeNames[LocaleDBC_Esp];
