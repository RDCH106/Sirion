...
public:
    ...
    static bool HandleGuildInviteCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (handler->GetSession()->GetSecurity() <= 1 && targetGuid != handler->GetSession()->GetPlayer()->GetGUID())
        {
            targetGuid = handler->GetSession()->GetPlayer()->GetGUID();
            ChatHandler chat(handler->GetSession());
            std::string msg = "Con tu nivel de GM, solo puedes invitarte a ti mismo";
    ...
    static bool HandleGuildUninviteCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (handler->GetSession()->GetSecurity() <= 1 && targetGuid != handler->GetSession()->GetPlayer()->GetGUID())
        {
            ...
            ChatHandler chat(handler->GetSession());
            std::string msg = "Con tu nivel de GM, solo puedes desinvitarte a ti mismo";
            chat.SendSysMessage(msg.c_str());
        }
	...
	static bool HandleGuildSetLevelCommand(ChatHandler* handler, char const* args)
	{
		...
		int32 newLevel = levelStr ? atoi(levelStr) : 0;
		if (newLevel <= 0)
			return false;
		...
		if (guild)
		{
			...
			{
				handler->PSendSysMessage("Hermandad al maximo nivel");
				return false;
			}
			else
		...
		{
			handler->PSendSysMessage("No se ha encontrado ningun Hermandad llamada [%s].", guildName);
			return false;
		}
	...
	static bool HandleGuildGiveXpCommand(ChatHandler* handler, char const* args)
	{
		...
		if (!guildStr)
			return false;
		char* levelStr = strtok(NULL, "");
		int32 value = levelStr ? atoi(levelStr) : 0;
		if (value <= 0)
			return false;
