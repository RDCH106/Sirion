...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "global",         rbac::RBAC_PERM_COMMAND_ANTICHEAT_GLOBAL,     false,  &HandleAntiCheatGlobalCommand,         "" },
            { "player",         rbac::RBAC_PERM_COMMAND_ANTICHEAT_PLAYER,     false,  &HandleAntiCheatPlayerCommand,         "" },
            { "delete",         rbac::RBAC_PERM_COMMAND_ANTICHEAT_DELETE,  false,  &HandleAntiCheatDeleteCommand,         "" },
            { "handle",         rbac::RBAC_PERM_COMMAND_ANTICHEAT_HANDLE,  false,  &HandleAntiCheatHandleCommand,         "" },
            { "jail",           rbac::RBAC_PERM_COMMAND_ANTICHEAT_JAIL,     false,  &HandleAnticheatJailCommand,         "" },
            { "warn",           rbac::RBAC_PERM_COMMAND_ANTICHEAT_WARN,     false,  &HandleAnticheatWarnCommand,         "" },
//            { NULL,             0,                     false, NULL,                                           "", NULL }
        };

        static std::vector<ChatCommand> commandTable =
        {
            { "anticheat",      rbac::RBAC_PERM_COMMAND_ANTICHEAT,     false, NULL,                     "",  anticheatCommandTable},
//            { NULL,             0,                  false, NULL,                               "", NULL }
        };
    ...
    static bool HandleAnticheatWarnCommand(ChatHandler* handler, const char* args)
    {
        ...
        char* buf = strdup("Te estamos vigilando...");
        char* pos = buf;
    ...
    static bool HandleAnticheatJailCommand(ChatHandler* handler, const char* args)
    {
        ...
        // teleport the target to jail.
        pTarget->TeleportTo(1,16226.5f,16403.6f,-64.5f,3.2f);
		#pragma region Freeze al player (HAY QUE PROBARLO)
		// Freeze the target
		if (pTarget == handler->GetSession()->GetPlayer())
		{
		...
		{
			Aura* freeze = pTarget->AddAura(9454, pTarget);
			if (freeze)
			{				
				handler->PSendSysMessage(LANG_COMMAND_FREEZE, pTarget->GetName().c_str());
				// save player
				pTarget->SaveToDB();				
			}
		}
#pragma endregion 
		// teleport the gm to jail.
        handler->GetSession()->GetPlayer()->TeleportTo(1,16218.83f,16403.6f,-64.5f,6.28f);
    ...
    static bool HandleAntiCheatPlayerCommand(ChatHandler* handler, const char* args)
    {
		...
		uint32 extern_programs = sAnticheatMgr->GetTypeReports(guid, 6);

        handler->PSendSysMessage("Informacion del jugador %s",player->GetName().c_str());
        handler->PSendSysMessage("Promedio: %f || Reportes totales: %u ",average,total_reports);
        handler->PSendSysMessage("Speed Reports: %u || Fly Reports: %u || Jump Reports: %u ",speed_reports,fly_reports,jump_reports);
		handler->PSendSysMessage("Walk On Water Reports: %u  || Teleport To Plane Reports: %u", waterwalk_reports, teleportplane_reports);
		handler->PSendSysMessage("External Programs: %u", extern_programs);
        handler->PSendSysMessage("Climb Reports: %u", climb_reports);
		//handler->PSendSysMessage("Reporte sobre programas externos: %u", extern_programs);
